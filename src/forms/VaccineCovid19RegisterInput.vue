...
.ep__registration-step.ep__registration-step--framed
  el-form.vaccine-covid19-register-userinfo(
    ...
    section(v-if="!is_searching")
      ...
      section
        el-row(:gutter="20")
          el-col(:sm="12")
            el-form-item(label="Họ tên người liên hệ")
        ...
        el-row(:gutter="20")
          el-col(:sm="12")
            el-form-item(label="Số di động người liên hệ")
      ...
      section(v-if="isShowCMND")
        el-row(:gutter="20")
          el-col(:sm="12")
            el-form-item(label="Số CMND/CCCD/Passport")
  ...
  el-form.vaccine-covid19-register-userinfo(
    ...
    section
      el-row(:gutter="20")
        el-col
          el-form-item(label="Vui lòng chọn đối tượng phù hợp với tình trạng người được tiêm")
      ...
      el-row(:gutter="20")
        el-col(:sm="24")
          el-form-item(label="Quý khách vui lòng ghi rõ tình trạng sức khoẻ của mình")
            single-choice(
      ...
      el-row(:gutter="20", v-if="form.benhnen == 'Có bệnh lý nền'")
        el-col(:sm="24")
          el-form-item(label="Chọn các loại bệnh lý gặp phải")
            el-checkbox-group(v-model="form.dsbenhnen")
              .underlying-item
                el-checkbox(
                  v-for="i, k in underlyings"
                  :key="k"
                  :value="i"
                  :label="i")
      el-row(:gutter="20")
        el-col(:sm="24")
          el-form-item(label="Người tiêm đã từng là F0?")
            single-choice(
      ...
      el-row(:gutter="20", v-if='false')
        el-col(:sm="24")
          el-form-item(label="14 ngày gần đây, Quý khách có sử dụng thuốc kháng sinh hoặc tiêm bất kỳ loại vắc xin nào không?")
            single-choice(
      ...
      el-row(:gutter="20", v-if='false')
        el-col(:sm="24")
          el-form-item(label="Quý khách có mong muốn đăng ký khám chuyên khoa khác cùng ngày hay không?")
            single-choice(
      ...
      el-row(:gutter="20", v-if="false && form.injected_subject && form.injected_subject.id === 'PREGNANT_WOMEN'")
        el-col(:sm="24")
          el-form-item(label="Người được tiêm có đang mang thai hay không?")
            single-choice(
      ...
      el-row(:gutter="20", v-if="form.injected_subject && form.injected_subject.id === 'PREGNANT_WOMEN'")
        el-col(:sm="24")
          el-form-item(label="Vui lòng nhập tuần thai người được tiêm")
            el-input(
                v-model="form.pregnant_week"
                placeholder="Nhập số tuần thai")
      el-row(:gutter="20", v-if="form.injected_subject && form.injected_subject.id === 'PREGNANT_WOMEN'")
        el-col(:sm="24")
          el-form-item(label="Người được tiêm có từng sinh con ở BV Tâm Anh hay không?")
            single-choice(
  ...
  el-form.vaccine-covid19-register-success(
    ...
    section
      ...
      el-col
        ...
        p(style="color: #0957DE; font-weight: 700; font-size: 16px; margin-top: 5px; margin-bottom: 10px;") {{ `${form.name ? form.name.toUpperCase() : ''}, ${form.phone}` }}
        p(v-if="form.pid", style="margin-top: 5px; margin-bottom: 10px;") PID:&nbsp;
          b {{ form.pid }}
        p(style="margin-top: 5px; margin-bottom: 10px;") {{ getPidBirthday() }}
        p(v-if="form.cmnd || form.cmnd_date || form.cmnd_location", style="margin-top: 5px; margin-bottom: 10px;") CMND&nbsp;{{  `${form.cmnd ? form.cmnd : ''}, cấp ngày ${form.cmnd_date ? getDateStr(form.cmnd_date) : ''}, tại ${form.cmnd_location ? form.cmnd_location.toUpperCase() : ''}` }}
      ...
      el-col
        ...
        p(style="color: #0957DE; font-weight: 700; font-size: 16px; margin-top: 5px; margin-bottom: 10px;") {{ `${form.injected_subject && form.injected_subject.name ? form.injected_subject.name : ''} / ${form.benhnen ? form.benhnen : ''}` }}
...
export default {
  ...
  methods: {
    ...
    getAddressFull () {
      ...
      let _ward = {name: ''}
      if (province_id) {
        _province = this.getNameByIdInList(this.provinces, province_id)
      }
      if (district_id) {
        _district = this.getNameByIdInList(this.districts, district_id)
      }
      if (war_id) {
        _ward = this.getNameByIdInList(this.wards, war_id)
      }
      return `${address ? address.toUpperCase() : ''}, ${_ward.name}, ${_district.name}, ${_province.name}`
    },
    ...
    saveRequest () {
      ...
      const _form = {
        ...
        province: _province ? _province.name : '',
        district: _district ? _district.name : '',
        ward: _ward ? _ward.name : '',
        country: this.getCountryName(),
        data_input_from: 'WEB_ADMIN_IFRAME'
      }
